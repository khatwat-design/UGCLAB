            ...
            VStack(spacing: 20) {
                if isLoading {
                    ...
                    VStack(spacing: 8) {
                        ...
                        if let pending = wallet?.pendingBalance, pending > 0 {
                            Text("\(Formatters.currency(pending)) معلق")
                                .font(.system(size: 12))
    ...
    private func loadWallet() {
        ...
        Task {
            do {
                ...
                await MainActor.run { self.wallet = wallet; transactions = txs; isLoading = false }
            } catch {
